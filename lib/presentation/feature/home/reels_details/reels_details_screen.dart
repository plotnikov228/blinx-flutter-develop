...
class _ReelsDetailsScreenState extends State<ReelsDetailsScreen> {
  ...
  Widget seeMore(String st,context) {
    final span=TextSpan(text:st,style: const TextStyle(
      color: Colors.white,
        fontSize: 16,
        fontWeight: FontWeight.w300
    ),);
    final tp =TextPainter(text:span,maxLines: 2,textDirection: TextDirection.rtl);
    tp.layout(maxWidth: MediaQuery.of(context).size.width-200); // equals the parent screen width

    if(tp.didExceedMaxLines) {
      return const Text('عرض المزيد',style: TextStyle(
          color: Colors.grey,
          fontSize: 14,
  ...
  Widget build(BuildContext context) {
    ...
    if(widget.articles.length!=vpControllers.length&&widget.articles.length>vpControllers.length) {
      List.generate(widget.articles.length-vpControllers.length, (index) => vpControllers.add(null));
    }
